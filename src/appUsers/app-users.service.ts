...
export class AppUsersService {
  ...
  async create(createAppUserDto: CreateAppUserDto) : Promise<object | any> {
    const sha256Password = createHash('sha256').update(createAppUserDto.password).digest('base64');
  ...
  async findOne(id: number) {
    const appUser : AppUser = (await this.appUserRepository.find({
      where: {
        id: id
      },
      relations: {
  ...
  async addNewContactInfoOnAppUser(id: number, createContactInfoDto: CreateContactInfoDto) {
  ...
  async update(id: number, updateAppUserDto: UpdateAppUserDto) {
    const sha256Password = createHash('sha256').update(updateAppUserDto.password).digest('base64');
  ...
  async addContactInfoOnUser(contactInfos : CreateContactInfoDto[], result : AppUser) {
