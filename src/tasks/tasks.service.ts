...
export class TasksService {
  ...
  async addTaskOnAnAppUser(appUserId: number, createTaskDto: CreateTaskDto) : Promise<object | string> {
    ...
    if( appUser === null)
      return `${appUserId} could not found!`;
  ...
  async addTaskOnATeam(teamId: number, createTaskDto: CreateTaskDto) : Promise<object | string> {
    const team : Team = (await this.teamsRepository.find( {
      where: {
        id: teamId,
        isActive: true
      }
